...
function ForgotPassword() {
  ...
  const handleInputChange = (e) => {
    ...
    const newState = isEmail
      ? { ...initialValues, [name]: newValue }
      : { ...initialPhone, [name]: newValue };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const res = isEmail
        ? await sendOtpByEmail(payload)
        : await sendOtpByPhone(payload);
  ...
  const handleOtpSubmit = async () => {
    ...
    if (isAnyBlank) {
      ...
      try {
        const res = isPhone
          ? await sendVerifyPhoneOtp(otpDatas)
          : await sendVerifyEmailOtp(otpDatas);
        if (!res.error) {
                        ...
                        <div className="form-row d-flex justify-content-between mt-4 mb-2">
                          ...
                          <div className="">
                            ...
                            >
                              {isLoading ? (
                                <small>
                                  <Spinner animation="grow" size="sm" /> OTP
                                  SENDING...
                                </small>
                              ) : otpSent ? (
                                timeLeft > 0 ? (
                    ...
                    <form onSubmit={handleSubmit}>
                      <>
                        ...
                        <div className="text-black text-center w-100">
                          Enter Your Mobile No With Country Code..!{" "}
                        </div>
                        <div className="form-row d-flex justify-content-between mt-4 mb-2">
                          <div className="">
                            <button
                    ...
                    <form onSubmit={handleOtpSubmit}>
                      <div className="mb-4 d-flex justify-content-between">
                        {otpDigits.map((digit, index) => (
                          <div
                            className="position-relative mx-2 otpInp"
                            key={index}
                          >
                            <CustomInputField
                              id={`otp${index}`}
              ...
              <div className="col-xl-6 col-lg-6 mx-auto align-self-center">
                <div className="pages-left h-100">
                  <div className="login-content">
                    ...
                    <p>
                      uses line charts to visualize customer-related metrics and
                      trends over time.
                    </p>
                  </div>
                  <div className="login-media text-center">
                    <img
                      src={
                        dataLg?.login_background_image
                          ? `${baseUrlImage}${dataLg?.login_background_image}`
                          : loginAbarisBanner
                      }
                      className="mb-3 logo-dark"
                      alt
